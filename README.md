...
設定後にアプリの「設定」からアカウントを作成してログインすると、メモとフォルダがクラウドへ保存されます。既存のローカルデータは、クラウド側が空の最初のログイン時に移行されます。
...
現在はブラウザ内への保存まで実装しています。PCとスマホ間の即時同期を行うには、次の段階で認証付きの共有バックエンド（ユーザー・フォルダ・メモ・添付ファイルを保存するAPI／データベース）を接続してください。UI側の保存処理は `persist()` にまとまっているため、同期APIへの置き換えがしやすい構成です。
